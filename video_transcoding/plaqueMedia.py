...
class Video():
  ...
  def Transcode(self):
    ...
    h264_transcode_task=VideoTranscodeTask(
      ...
      transcoded_video,
      [
        "ffmpeg -y -i \"src_video/{source_video}\" -vf \"scale=1280:720:force_original_aspect_ratio=decrease,pad=1280:720:-1:-1:color=black\" \"transcoded_video/{transcoded_video}.png\"",
        "az storage azcopy blob upload -c \"media\" --account-name plaquetv -s \"transcoded_video/{transcoded_video}.*\" " 
      ],
      )
    ...
    h264_transcode_task=VideoTranscodeTask(
      ...
      transcoded_video,
      [
        "ffmpeg -y -i \"src_video/{source_video}\" -vf \"scale=1280:720:force_original_aspect_ratio=decrease,pad=1280:720:-1:-1:color=black\" -c:v libx264 -preset veryslow -crf 14 -c:a aac -b:a 192k -pix_fmt yuv420p -movflags +faststart \"transcoded_video/{transcoded_video}.png\"",
